function varargout=admiplos(thhats,th0,truecov,E,v,params,ah,numsim)
% [ah,ha,yl]=admiplos(thhats,th0,truecov,E,v,params,ah,numsim)
%
...
% the admittance given these data. As seen in Simons & Olhede (2013)
%
% INPUT:
%
...
% numsim     The original number of simulations from which this sample is drawn
%
% OUTPUT:
%
...
% EXAMPLE:
%
% This only gets used in MLEOS/MLEROS thus far
%
% SEE ALSO:
%
...
if length(th0)==5
  ...
  tD =th0(:,1);
  tf2=th0(:,2);
  % And then the correlation coefficient is zero!
  tr=0;
  ts2=th0(:,3);
  tnu=th0(:,4);
  trh=th0(:,5);
...
tTe=DtoTe(tD,E,v);
[tG2b,k,l,tZb,Zf]=forsyth(tTe,lbd,tf2,tr,DEL(1),DEL(2),z2,g,1);
...
for ind=1:length(Te)
  [G2b(:,ind),~,l,Zb(:,ind)]=...
      forsyth(Te(ind),lbd,f2(ind),r(ind),DEL(1),DEL(2),z2,g);
end
...
set(ah,'xtick',thth,'xticklabel',thth)
...
for ind=1:length(thhats)*2
  ...
  elseif length(th0)==6
    [T,G,th0dd,p,k,Hk,Gk]=simulros(th0,pars,pars.blurs);
  end
