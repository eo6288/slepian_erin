...
if ~isstr(Hx)
  ...
  %logliros (as opposed to logliosl) doesn't have the ability to make
  % vr
  [~, ~,momx]=logliros(scl.*thhat,params,Hk,k,scl);
...
elseif strcmp(Hx,'demo1')
  ...
  for index=1:N
	...
	avH=avH+lpars{3}; %mleosl.m has this (divided by) ./[scl(:)*scl(:)'];
	...
	%[L,~,~,momx, ~, ~]=logliosl(k,thhat,p,Hk); %Hk not defined yet
  ...
  if good>=1 
    ...
    [covF,F]=covthros(th0./sclth0,p,k,sclth0); %mleosl.m uses fishios.m
